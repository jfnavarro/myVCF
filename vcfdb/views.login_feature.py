...
def display_vcf_table(request, gene_ensgene):
    ...
    if request.method == "GET":
        ...
        context = {'samples_col': samples_col, 'default_col': default_col,
                   'all_fields': all_fields, 'gene': gene, 'mutations': mutations}
        return render(request, 'gene_results.html', context)


def VcfToJson(request):
    query = request.POST['q_json']
    object_list = Vcf.objects.all().filter(gene_refgene__icontains=query)
...
def get_gene_mutations(request, gene_ensgene):
    if request.method == "GET":
        gene = gene_ensgene
        mutations = Vcf.objects.filter(gene_ensgene__icontains=gene).values('chrom', 'rs_id', 'ref', 'alt', 'af',
                                                                            'exonicfunc_refgene', 'gene_refgene',
                                                                            'gene_ensgene', 'h32').distinct()
